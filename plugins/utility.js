...
cmd({
    ...
    try {
        ...
        for (const img of images) {
            await conn.sendMessage(from, { image: { url: img }, caption: '> © ᴘᴏᴡᴇʀᴇᴅ ʙʏ 𝙽𝙰𝚆𝙰𝚉 𝙼𝙳' }, { quoted: mek });
        }
    } catch (e) {
        console.log(e);
        reply(`${e}`);
...
cmd({
    ...
    try {
        ...
        const response = await axios.get(apiUrl);
        const data = response.data;
        await conn.sendMessage(from, { image: { url: data.message }, caption: '> *© Powered By 𝙽𝙰𝚆𝙰𝚉 𝙼𝙳*' }, { quoted: mek });
    } catch (e) {
        console.log(e);
